-- Phase 29.2 - Mobile sync, Admin role, and email template fix
-- Run this in Supabase SQL editor after uploading this build.

-- Make sure Anil / Aashan owner accounts show as Admin on every device.
update user_profiles
set role = 'Admin', active = true
where lower(email) in ('thomasmathew77@gmail.com', 'support@aashan.co');

-- Keep Master email templates updated with customer name merge field.
do $$
begin
...
Website: www.aashan.co'
  ...
  if not found then
    ...
    values ('Quote Email', 'Quote {{quote_no}} from Aashan & Co LLC', 'Hi {{customer_name}},
...
Website: www.aashan.co'
  ...
  if not found then
    insert into email_templates (template_name, subject, body)
    values ('Invoice Email', 'Invoice {{invoice_no}} from Aashan & Co LLC', 'Hi {{customer_name}},
...
Aashan & Co LLC'
  ...
  if not found then
    insert into email_templates (template_name, subject, body)
    values ('Payment Receipt Email', 'Payment Receipt {{receipt_no}} from Aashan & Co LLC', 'Hi {{customer_name}},
